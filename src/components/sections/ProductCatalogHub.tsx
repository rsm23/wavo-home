...
export default function ProductCatalogHub() {
  ...
  return (
    <section id="produits" className="relative z-20 py-28 bg-slate-50/70 dark:bg-[#080b13] border-t border-slate-200/80 dark:border-white/[0.08] transition-colors duration-300">
          ...
          {filteredCategories.map((item, idx) => {
            ...
            return (
              <FloatingCard3D key={item.id} className="h-full">
                <div className="h-full p-7 rounded-3xl wavo-card flex flex-col justify-between space-y-6 group relative overflow-hidden">
                  
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="p-3 rounded-2xl bg-slate-100 dark:bg-white/[0.04] text-[#fa6e69] border border-slate-200/80 dark:border-white/[0.08] group-hover:scale-110 group-hover:text-[#ffbc7d] transition-all">
                        <Icon className="w-5 h-5" />
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-slate-100 dark:bg-white/[0.05] text-slate-700 dark:text-slate-300 border border-slate-200/60 dark:border-white/[0.08]">
                          {grade}
                        </span>
                        <span className="px-3 py-1 rounded-full text-[11px] font-mono font-semibold bg-[#fa6e69]/10 dark:bg-[#fa6e69]/15 text-[#fa6e69] border border-[#fa6e69]/30">
                          &lt; {item.rotation}
                        </span>
                      </div>
                    </div>
